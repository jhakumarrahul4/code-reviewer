// this component handles the github url input bar
const GithubBar = ({
  githubUrl,
  setGithubUrl,
  githubLoading,
    ...
    <div className="github-bar">
      <div className="github-input-row">
        ...
        <input
          ...
          placeholder="Paste GitHub file URL and press Enter..."
          value={githubUrl}
      ...
      {/* success message after file loads */}
      {fetchedFile && !githubLoading && (
        <div className="github-success">
          ✅ Loaded: <span>{fetchedFile}</span>
        </div>
      )}

      {/* error message if fetch fails */}
      {githubError && !githubLoading && (
        <div className="github-error">⚠️ {githubError}</div>
      )}
    </div>
  );
};
